    ...
    <>
      ...
      {!isSeats && <div className='w-screen justify-center items-center flex h-screen bg-gradient-to-r from-purple-500 to-pink-500'>
        <div className='bg-white w-[70%] rounded-lg flex flex-col'>
          ...
          <button onClick={() => {
            let data = {}; if (!notMsg && price.current.value && from && air.current.value && dept.current.value && dur.current.value && arr.current.value && to && img && row && id && (premium ? (busiprice && premprice) : true)) {
              data = {
              ...
              }; setDataf(data); setIsSeats(true)
            } else { window.alert('Enetr All the Details') }; console.log(price.current.value)
          }} className='w-28 text-white mt-2 mb-2 self-center bg-blue-700 h-10 rounded-3xl hover:scale-110 hover:bg-brightness-75 ' >Register</button>
